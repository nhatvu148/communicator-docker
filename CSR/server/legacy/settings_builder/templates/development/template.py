...
display_name = u'Development Mode'

description = u'Configure the entire system to run on a single machine with a built in web server.  This configuration is not for production use.'
...
FILE_SERVER_HOST_PORT_INDEX = 8
EVENT_LOG_HOST_PORT_INDEX = 10
SERVICE_BROKER_HOST_PORT_INDEX = 11
SC_SERVER_SSL_CONFIGURE_INDEX = 18
SC_SERVER_SSL_RESPONSE_INDEX = 19
    ...
    def __init__(self, interaction):
        ...
        self.fields = [
            ...
            self.interaction.MultiplePathField(
                u'Enter an absolute path to the file server directory',
                u'<directory>{FILE_SERVER_DIRECTORY}</directory>',
                u'{FILE_SERVER_DIRECTORY}',
                minimum = 1,
                maximum = None,
                default_value = [self.interaction.complete_path(setting) for setting in self.interaction.get_command_line_setting(u'{FILE_SERVER_DIRECTORY}', None, [])]
                ),

            #Field 8 - FILE_SERVER_HOST_PORT_INDEX
            self.interaction.ConstantField(
                u'File server host port',
                u'{FILE_SERVER_HOST_PORT}',
                self.interaction.get_command_line_setting(u'{FILE_SERVER_HOST_PORT}') or FILE_SERVER_HOST_PORT
                ),
            ...
            #Field 10 - EVENT_LOG_HOST_PORT_INDEX
            self.interaction.ConstantField(
            ...
            #Field 11 - SERVICE_BROKER_HOST_PORT_INDEX
            self.interaction.ConstantField(
            ...
            #Field 18 - SC_SERVER_SSL_CONFIGURE_INDEX
            self.interaction.BooleanField(
            ...
            #Field 19 - SC_SERVER_SSL_RESPONSE_INDEX
            self.interaction.ConstantField(
                u'SSL Response',
                u'{SSL_RESPONSE}',
                0
