...
// Local imports
import * as createServers from './CreateServers';
import * as spawnerTypes from './SpawnerTypes';
import {AppConfig, IPVersion, UserConfig} from './SpawnerTypes';
import {createLogger, logger} from "./Logging";
import {expandHomeDir, getHttpProtocol, getLocalHostname, resolveDirs} from "./Utils";
import {LogLevel} from "./SpawnerTypes";
...
let gShutdownCode: null | number = null;
let gFileServer: null | http.Server | https.Server = null;
let gSpawnServer: null | http.Server | https.Server = null;
...
function main() {
    ...
    try {
        ...
        process.on('uncaughtException' as any, err => {
            logger().error(`Fatal application error: ${err}`);
            triggerShutdown(1);
        });
...
function readConfigAndSetupLogger(): UserConfig {
    ...
    if (!hasLogConsoleLevelCutoff) { 
        ...
        userConfig.logConsoleLevelCutoff = LogLevel.Warn;
    }
    ...
    if (!hasLogConsoleLevelCutoff) {
        logger().warn(`Configuration optional setting 'logConsoleLevelCutoff' is undefined. Recommend adding with the default value.`);
    }
...
function launchServers(appConfig: AppConfig) {
    // Validate the node version now that we can log
    const nodeVersion = process.version;
    if (nodeVersion !== kShippingNodeVersion) {
        logger().warn(`This server has been tested to work with NodeJS ${kShippingNodeVersion}, but is being run with ${nodeVersion}`);
        const currentMajorVersion = Number(process.version.match(/^v(\d+\.\d+)/)[1]);
        if (currentMajorVersion < kKnownCompatibleMajorVersion) {
            logger().error(`This server will not work with NodeJS versions prior to ${kKnownCompatibleMajorVersion}`);
            triggerShutdown(1);
    ...
    if (appConfig.fileServerPort) {
        ...
        gFileServer.on('error', (error: any) => {
            onHttpServerError(error, appConfig.fileServerPort);
        });

        gFileServer.on('listening', function () {
            const server = this as http.Server | https.Server;
            const useSsl = server instanceof https.Server;
            const port = (server.address() as AddressInfo).port;
            logger().info(`HTTP file-server listening: port=${port}, SSL=${useSsl}`);
    ...
    if (appConfig.spawnServerPort) {
        ...
        gSpawnServer.on('error', (error: any) => {
            onHttpServerError(error, appConfig.spawnServerPort);
        });

        gSpawnServer.on('listening', function () {
            const ssHttpProtocol = getHttpProtocol(appConfig.sslEnableSpawnServer);
            const ssHostname = getLocalHostname(appConfig.sslEnableSpawnServer, appConfig.publicHostname, appConfig.ipVersion);

            const curlExe = (os.platform() === "win32") ? "curl.exe" : "curl";
            logger().info(`Server help available via HTTP GET to endpoint api/help. ex:`);
            logger().info(`  ${curlExe} ${ssHttpProtocol}://${ssHostname}:${appConfig.spawnServerPort}/api/help`);
            if (appConfig.disableConsoleEnterToShutdown) {
                logger().info(`The server can be shutdown with an HTTP POST to api/shutdown. ex:`);
                logger().info(`  ${curlExe} -X POST ${ssHttpProtocol}://${ssHostname}:${appConfig.spawnServerPort}/api/shutdown`);
            }

            const server = this as http.Server | https.Server;
            const useSsl = server instanceof https.Server;
            const port = (server.address() as AddressInfo).port;
            logger().info(`REST spawn-server listening: port=${port}, SSL=${useSsl}`);
            onServerListening(appConfig);
        });
    ...
    if (appConfig.autoGcIntervalTime) {
        if (global.gc) {
            ...
            logger().info(`Periodic GC enabled every ${appConfig.autoGcIntervalTime} seconds`);
        } else {
            logger().warn(`autoGcIntervalTime is configured, but the garbage collector is disabled.`);
        }
    }
}

/** Cause the given server to start listening. Uses the IpVersion to determine how it listens */
function listenWithServer(server: http.Server, port: number, ipVersion: spawnerTypes.IPVersion) {
    ...
    if (!ipVersion || ipVersion === IPVersion.Auto) {
        // Auto mode for ipv4/ipv6
        server.listen(port);
    } else if (ipVersion === IPVersion.ForceIPv4) {
        server.listen(port, "0.0.0.0");
    } else if (ipVersion === IPVersion.ForceIPv6Disabled) {
        server.listen(port, "::");
    } else {
...
function onServerListening(userConfig: spawnerTypes.UserConfig) {
    if (--gStartCount === 0) {
...
function onHttpServerError(error: any, port: number): void {
    ...
    switch (error.code) {
        case 'EACCES':
            logger().error(`Startup: ${port} requires elevated privileges`);
            triggerShutdown(1);
            break;
        case 'EADDRINUSE':
...
function loadUserConfig(): spawnerTypes.UserConfig {
    const args = program
        .version('1.0.0')
    ...
    try {
        return require(args.configFile) as spawnerTypes.UserConfig;
    } catch (err) {
...
function errorIfSettingMissing(name: string, userConfig: any) {
...
/** Validate that a named user-config setting has been specified */
function warnIfSettingMissing(name: string, userConfig: any) {
    if (!userConfig.hasOwnProperty(name)) {
        logger().warn(`Configuration optional setting '${name}' is undefined. Recommend adding with the default value.`);
    } else {
        // The setting exists, but still ensure the associated value isn't "undefined". 
        if (userConfig[name] === undefined) {
            logger().warn(`The value for configuration setting '${name}' is undefined, check it for typos or incorrect enum values`);
        }
    }
}

/** Validate that the user config is reasonable */
function createAppConfigFromUserConfig(userConfig: spawnerTypes.UserConfig,
                                       serverDir: string,
                                       shutdownCallback: (exitCode: number) => void): AppConfig {
    ...
    const commDir = expandHomeDir(userConfig.communicatorDir);
    ...
    if (userConfig.ipVersion === IPVersion.ForceIPv6Disabled) {
        throw new Error(`Explicit IPv6 support is disabled at this time, the ipVersion setting must be changed`);
    }

    const sslPrivateKeyAbsFile = userConfig.sslPrivateKeyFile ? resolveDirs(commDir, userConfig.sslPrivateKeyFile) : null;
    const sslCertificateAbsFile = userConfig.sslCertificateFile ? resolveDirs(commDir, userConfig.sslCertificateFile) : null;
    const ssProtocol = getHttpProtocol(userConfig.sslEnableSpawnServer);
    const ssHostname = getLocalHostname(userConfig.sslEnableSpawnServer, userConfig.publicHostname, userConfig.ipVersion);
    ...
    const appConfig: AppConfig = {
        ...
        serverDir: serverDir,
        communicatorAbsDir: commAbsDir,
        spawnExeAbsFile: resolveDirs(commDir, (() => {
        ...
        logAbsDir: userConfig.logDir ? resolveDirs(commDir, userConfig.logDir) : null,
        modelAbsDirs: userConfig.modelDirs ? userConfig.modelDirs.map(p => resolveDirs(commDir, p)) : null,
        workspaceAbsDir: userConfig.workspaceDir ? resolveDirs(commDir, userConfig.workspaceDir) : null,
        licenseAbsFile: userConfig.licenseFile ? resolveDirs(commDir, userConfig.licenseFile) : null,
        statusUriBase: `${ssProtocol}://${ssHostname}:${userConfig.spawnServerPort}/api/spawns/`,
        fileServerStaticAbsDirs: userConfig.fileServerStaticDirs ? userConfig.fileServerStaticDirs.map(p => resolveDirs(commDir, p)) : null,
        sslPrivateKeyAbsFile: sslPrivateKeyAbsFile,
        sslCertificateAbsFile: sslCertificateAbsFile,
...
function triggerShutdown(code: number) {
    ...
    setTimeout(() => {
        ...
        if (gShutdownCode === null) {
            gShutdownCode = code;
            logger().info(`Exiting server with code ${gShutdownCode}`);
        }
